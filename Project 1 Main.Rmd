...
```{r scatterplot of total pay and duration, echo=FALSE}
...
plot(
  df_merged_2$Salary,
  df_merged_2$Total,
  main = "Scatterplot Between Salary and Number of 311 Cases, by Borough Boards",
...
```
So as one final chance, lets try grouping these by distric and see if there is a stronger result. This removes smaller positions like 
...
```{r}
#Last I want to compare distric manager to number of cases
```
